...
const gradients = [
  'linear-gradient(135deg,#6B5BFF,#4F7EFF)',
  'linear-gradient(135deg,#4DD39A,#6B5BFF)',
  'linear-gradient(135deg,#F5A623,#F38288)',
  'linear-gradient(135deg,#F38288,#6B5BFF)',
...
// KPI summary derived from current rows
const totalGross = computed(() => filteredRows.value.reduce((s, r) => s + gross(r), 0))
const totalNet = computed(() => filteredRows.value.reduce((s, r) => s + net(r), 0))
const totalDeductions = computed(() => filteredRows.value.reduce((s, r) => s + deductions(r), 0))
</script>
...
<template>
  <div class="slips-page">
    ...
    <div class="page-head">
      <div class="ph-text">
        <div class="ph-eyebrow">Performance &amp; Pay · Payslips</div>
        <h1 class="ph-title">Payslips</h1>
        <p class="ph-sub">Review, preview, and distribute employee payslips for the selected pay cycle.</p>
      </div>
    ...
    <div class="kpi-strip">
      <div class="kpi">
        ...
        <div class="kpi-sub">{{ monthF || 'All months' }}</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">Gross payout</div>
        <div class="kpi-value accent">{{ fmt(totalGross) }}</div>
        <div class="kpi-sub">across {{ filteredRows.length }} employees</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">Deductions</div>
        <div class="kpi-value yellow">{{ fmt(totalDeductions) }}</div>
        <div class="kpi-sub">PF · TDS · ESI</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">Net disbursal</div>
        <div class="kpi-value green">{{ fmt(totalNet) }}</div>
        <div class="kpi-sub">total take-home</div>
      </div>
    </div>
    ...
    <div class="filters">
      <div class="s-box">
        <svg width="13" height="13" viewBox="0 0 20 20" fill="currentColor" class="search-icon">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(row, i) in filteredRows" :key="row.id">
              ...
              <td class="mono num">{{ fmt(gross(row)) }}</td>
              <td class="mono num red">−{{ fmt(deductions(row)) }}</td>
              <td class="mono num bold">{{ fmt(net(row)) }}</td>
              <td><StatusPill :status="statusOf(row)" /></td>
              <td class="num">
                <div class="row-acts">
                  <button class="ra-btn" @click="viewSlip(row)">View</button>
                  <button class="ra-btn" @click="downloadSlip(row)">Download</button>
                </div>
              </td>
            </tr>
...
.kpi-value.accent { color: #6B5BFF; }
.kpi-value.yellow { color: #F5A623; }
.kpi-value.green { color: #4DD39A; }
.kpi-sub {
  font-size: 11px;
  color: #7A8299;
  margin-top: 2px;
