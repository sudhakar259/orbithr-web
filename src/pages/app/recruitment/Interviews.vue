...
import { recruitmentService, type Interview } from '@/services/recruitmentService'
...
const submitForm = async () => {
  ...
  try {
    ...
    form.value = { application_id: undefined, title: '', interview_type: 'video', scheduled_at: '', duration_minutes: 60, location: '', meeting_link: '', interviewer_ids: [] }
    await load()
  } catch (e: unknown) {
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="showForm" class="bg-gray-800 border border-gray-700 rounded-lg p-6 space-y-4">
      <h3 class="text-sm font-semibold text-gray-300 uppercase tracking-wider">Schedule Interview</h3>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input v-model.number="form.application_id" type="number" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none" placeholder="Enter application ID" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Title *</label>
          <input v-model="form.title" type="text" placeholder="e.g. Technical Round 1" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Type</label>
          <select v-model="form.interview_type" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none">
        ...
        <div>
          ...
          <input v-model="form.scheduled_at" type="datetime-local" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Duration (minutes)</label>
          <input v-model.number="form.duration_minutes" type="number" min="15" step="15" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Location / Meeting Link</label>
          <input v-model="form.meeting_link" type="text" placeholder="Zoom/Meet URL or office location" class="bg-gray-700 border border-gray-600 text-white rounded-lg text-sm px-3 py-2 w-full focus:border-blue-500 focus:outline-none" />
        </div>
      </div>
      <div class="flex gap-3">
        <button @click="submitForm" :disabled="saving" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
    ...
    <!-- Empty -->
    <div v-else-if="interviews.length === 0" class="bg-gray-800 border border-gray-700 rounded-lg p-12 text-center">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="flex items-center gap-3">
                ...
                <span :class="['inline-flex px-2 py-0.5 text-xs font-semibold rounded-full', getStatusClasses(iv.status)]">
                  {{ iv.status.replace('_', ' ') }}
                </span>
              </div>
              <p class="text-sm text-gray-400 mt-0.5">{{ formatDateTime(iv.scheduled_at) }} · {{ iv.duration_minutes }}min</p>
              <p v-if="iv.meeting_link" class="text-xs text-blue-400 mt-0.5">
                <a :href="iv.meeting_link" target="_blank" rel="noopener">{{ iv.meeting_link }}</a>
              </p>
              <div v-if="iv.interviewers?.length" class="flex items-center gap-1 mt-1">
                <span class="text-xs text-gray-500">Interviewers:</span>
                <span v-for="iw in iv.interviewers" :key="iw.id" class="text-xs text-gray-300">{{ iw.user?.name ?? `User ${iw.user_id}` }}</span>
              </div>
            </div>
          </div>
