...
onMounted(async () => {
  ...
  if (error) {
    status.value = 'error'
    message.value = `Google denied access: ${error}`
    window.opener?.postMessage({ type: 'google-oauth-error', message: message.value }, '*')
  ...
  try {
    const res = await api.post('/calendar/google/exchange-code', { code })
    status.value = 'success'
    message.value = 'Google Calendar connected!'
    window.opener?.postMessage({ type: 'google-oauth-success', integration: res.data?.data }, '*')
    setTimeout(() => window.close(), 1500)
  } catch (e: unknown) {
    const err = e as { response?: { data?: { message?: string } } }
    status.value = 'error'
    message.value = err.response?.data?.message ?? 'Failed to connect Google Calendar.'
    window.opener?.postMessage({ type: 'google-oauth-error', message: message.value }, '*')
    setTimeout(() => window.close(), 2000)
  }
})
  ...
  <div style="min-height:100vh;background:#0C0E14;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;">
    <div style="background:#141720;border:1px solid #2A3050;border-radius:12px;padding:32px 40px;text-align:center;max-width:320px;width:100%;">
      <div v-if="status === 'loading'" style="color:#E8EAF0;">
        ...
        <p style="margin:0;font-size:14px;">Connecting Google Calendar…</p>
      </div>
